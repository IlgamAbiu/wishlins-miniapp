 ...
 * Uses Telegram HapticFeedback on interactions.
 */
...
import { useUser, subscribeVersion } from '@/composables/useUser'
import { useHaptic } from '@/composables/useHaptic'

const router = useRouter()
const { webapp, user } = useTelegramWebApp()
const { getFriends, searchUsers } = useUser()
...
const displayedUsers = computed(() => {
  if (searchQuery.value && searchResults.value.length > 0) return searchResults.value
...
async function fetchFriendsList() {
  ...
  try {
    ...
    error.value = null
    friends.value = await getFriends(user.value.id)
  } catch (e) {
...
async function handleSearch() {
  if (!searchQuery.value || !user.value) {
    searchResults.value = []
    return
  }
  ...
  try {
    const results = await searchUsers(searchQuery.value, user.value.id)
...
watch(subscribeVersion, () => fetchFriendsList())
</script>
  ...
  <div class="friends-view-stack" @click="handleBackgroundClick">
    <div class="friends-list-view">
      ...
      <div v-else-if="error" class="friends-view__error">
        <p>{{ error }}</p>
        <button class="primary-button" @click="fetchFriendsList">Попробовать снова</button>
      </div>

      <!-- Empty (no friends, not searching) -->
...
.friends-view__add-icon { font-size: 22px; line-height: 1; }

/* Search Bar */
.search-bar-wrapper { margin-bottom: 8px; }
...
[data-theme='dark'] .search-bar:focus-within { background: rgba(30, 41, 59, 0.7); }
...
.search-icon { color: var(--tg-hint-color); font-size: 20px; margin-right: 12px; opacity: 0.7; }

.search-input {
  flex: 1; background: transparent; border: none;
  color: var(--tg-text-color); font-size: 16px; font-weight: 400; outline: none; width: 100%;
}

.search-input::placeholder { color: var(--tg-hint-color); opacity: 0.6; }

.search-empty { height: 50vh; }
...
.friends-view__error {
  ...
  padding: 40px; text-align: center; gap: 16px; color: var(--status-error);
}
...
.placeholder__icon-wrapper { position: relative; display: inline-block; margin-bottom: var(--spacing-lg); }

.placeholder__icon {
  font-size: 72px; animation: gift-bounce 2.5s ease-in-out infinite;
...
.spin { animation: spin 1s linear infinite; font-size: 32px; color: var(--tg-button-color); }
